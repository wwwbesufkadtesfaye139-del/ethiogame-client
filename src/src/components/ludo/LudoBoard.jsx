...
/* ── Entry arrows ──────────────────────────────────────────────────────────
 * Derived directly from PATH_CELLS + START_POSITIONS (not eyeballed): each
 * color's arrow sits just outside the board, level with its true entry cell,
 * pointing the direction that piece actually travels first. Because this
 * board keeps the current (clockwise-correct) corner layout rather than the
 * reference image's literal corner colors, the *positions* below match the
 * image's pattern (one arrow per side) but the *colors* attached to each side
 * follow our corners instead — see chat for why. */
const ARROW_MARGIN = 22;
const ENTRY_ARROWS = [
  { color: 'red',    x: -ARROW_MARGIN,  y: 6*SZ + SZ/2,  rotate: 0   }, // left edge, pointing right
  { color: 'blue',   x: 8*SZ + SZ/2,    y: -ARROW_MARGIN, rotate: 90  }, // top edge, pointing down
  { color: 'green',  x: W + ARROW_MARGIN, y: 8*SZ + SZ/2, rotate: 180 }, // right edge, pointing left
  { color: 'yellow', x: 6*SZ + SZ/2,   y: W + ARROW_MARGIN, rotate: 270 }, // bottom edge, pointing up
];
  ...
  return (
    <g key={`${r}-${c}`}>
      ...
      {isSafe && !isHome && (
        <text
          x={x + SZ/2} y={y + SZ/2 + 6}
          textAnchor="middle" fontSize={18} fill="#E4E9F2" opacity={0.85}
...
/* ── Map-pin token shape ───────────────────────────────────────────────────
 * The reference board uses classic map-pin/location-marker tokens (round
 * head, tapered point, white halo ring, white "hole"), not flat circles.
 * Built as a path formula (not a hand-traced SVG import) so it scales
 * cleanly with headR and stays crisp at any board size. */
const PIN_HEAD_R = SZ * 0.30;
function pinPath(headR) {
  const tip = headR * 2.05;
  const k = headR * 0.55; // cubic-bezier control offset approximating a circle
  return `M0,${-headR} ` +
    `C${k},${-headR} ${headR},${-k} ${headR},0 ` +
    `C${headR},${headR*0.75} ${headR*0.3},${headR*1.4} 0,${tip} ` +
    `C${-headR*0.3},${headR*1.4} ${-headR},${headR*0.75} ${-headR},0 ` +
    `C${-headR},${-k} ${-k},${-headR} 0,${-headR} Z`;
}
const PIN_D = pinPath(PIN_HEAD_R);
const PIN_HALO_D = pinPath(PIN_HEAD_R * 1.2);
  ...
  return (
    ...
    >
      ...
      <g className={canTap ? 'anim-token-select' : undefined}>
        {/* Grounded shadow beneath the pin's point */}
        <ellipse cx={1} cy={r*1.9} rx={r*0.7} ry={r*0.24} fill="#00000055" />
        {/* White halo ring — a slightly larger duplicate sitting behind the
            coloured pin, matching the reference's white outline per token */}
        <path d={PIN_HALO_D} fill="#FFFFFF" opacity={0.95} />
        {/* Coloured pin body — gradient, not flat */}
        <path d={PIN_D} fill={`url(#tokenGrad-${color})`} stroke={col.dark} strokeWidth={1.4} />
        {/* Shine */}
        <ellipse cx={-r*0.28} cy={-r*0.32} rx={r*0.32} ry={r*0.2} fill="#ffffff70" />
        {/* Crown for a finished piece, otherwise the classic pin "hole" */}
        {isKing ? (
          <text x={0} y={5} textAnchor="middle" fontSize={r*1.1} fill="#FFD700"
            style={{userSelect:'none', fontWeight:900}}>♛</text>
        ) : (
          <circle cx={0} cy={-r*0.05} r={r*0.4} fill="#FFFFFF" stroke={col.dark} strokeWidth={0.6}/>
        )}
      </g>
    </motion.g>
  );
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="w-full rounded-2xl overflow-hidden"
        ...
        <svg
          viewBox={`${-ARROW_MARGIN-11} ${-ARROW_MARGIN-11} ${W+2*(ARROW_MARGIN+11)} ${W+2*(ARROW_MARGIN+11)}`}
          width="100%"
        ...
        >
          ...
          {/* ── Entry arrows, just outside the board ──────────────────── */}
          ...
          {/* ── Centre pinwheel — 4 triangles, each colour's home column
                 feeds in from its own side (left=red, top=blue, right=green,
                 bottom=yellow — matches HOME_RUNWAY below), with a small
                 chevron pointing further inward, per the reference image ── */}
          <g>
            {/* Left wedge — red */}
            <polygon points={`${6*SZ},${6*SZ} ${7.5*SZ},${7.5*SZ} ${6*SZ},${9*SZ}`} fill={C.red.main} />
            {/* Top wedge — blue */}
            <polygon points={`${9*SZ},${6*SZ} ${7.5*SZ},${7.5*SZ} ${6*SZ},${6*SZ}`} fill={C.blue.main} />
            {/* Right wedge — green */}
            <polygon points={`${9*SZ},${9*SZ} ${7.5*SZ},${7.5*SZ} ${9*SZ},${6*SZ}`} fill={C.green.main} />
            {/* Bottom wedge — yellow */}
            <polygon points={`${6*SZ},${9*SZ} ${7.5*SZ},${7.5*SZ} ${9*SZ},${9*SZ}`} fill={C.yellow.main} />

            {/* Inward chevrons — same small triangle used for the entry
                arrows, just darker (so it embosses rather than clashes) */}
            <polygon points="-6,-7 7,0 -6,7" fill={C.red.dark}    opacity={0.8} transform={`translate(${6.65*SZ},${7.5*SZ}) rotate(0)`} />
            <polygon points="-6,-7 7,0 -6,7" fill={C.blue.dark}   opacity={0.8} transform={`translate(${7.5*SZ},${6.65*SZ}) rotate(90)`} />
            <polygon points="-6,-7 7,0 -6,7" fill={C.green.dark}  opacity={0.8} transform={`translate(${8.35*SZ},${7.5*SZ}) rotate(180)`} />
            <polygon points="-6,-7 7,0 -6,7" fill={C.yellow.dark} opacity={0.8} transform={`translate(${7.5*SZ},${8.35*SZ}) rotate(270)`} />
          </g>
